...
export default function PdfPreview({
  ...
  const [mostrarSeleccionarChat, setMostrarSeleccionarChat] = useState(false)
  const [enviandoWa, setEnviandoWa] = useState(false)
  const { enviarConAdjunto: enviarWaAdjunto } = useWhatsapp()
  ...
  const handleSeleccionarChatWa = async (chat) => {
    setMostrarSeleccionarChat(false)
    if (!blobUrl) return
    setEnviandoWa(true)
    try {
      const blob = await fetch(blobUrl).then((r) => r.blob())
      const archivo = new File([blob], `${cotizacion.consecutivo}.pdf`, { type: 'application/pdf' })
      const formData = new FormData()
      formData.append('archivo', archivo)
      formData.append('texto', `Hola, te compartimos la cotización ${cotizacion.consecutivo} generada por ELECTROMANFER LTDA. Quedamos atentos.`)
      await enviarWaAdjunto(chat.id, formData)
    } catch (e) {
      console.error('Error enviando cotización por WhatsApp:', e)
    } finally {
      setEnviandoWa(false)
    }
  }
  ...
  return (
    <>
      <div className="pdf-preview">
        ...
        <div className="pdf-preview__toolbar">
          ...
          <div className="pdf-preview__toolbar-actions">
            ...
            <button
              ...
              onClick={() => { setMostrarEmailModal(true); setMostrarSeleccionarChat(false) }}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <button
              ...
              onClick={() => { setMostrarSeleccionarChat(true); setMostrarEmailModal(false) }}
              disabled={!blobUrl || enviandoWa}
            >
        ...
        {mostrarSeleccionarChat && (
          <SeleccionarChatModal
            titulo={`Enviar cotización ${cotizacion.consecutivo}`}
            onSeleccionar={handleSeleccionarChatWa}
            onClose={() => setMostrarSeleccionarChat(false)}
          />
        )}
